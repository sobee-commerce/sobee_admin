...
import {cn, fromFileSizeToReadable} from '@/lib/utils';
...
const ViaDevice = ({
  ...
  const onFilesChange = (_files: File[] | FileList | null) => {
    ...
    const merged = [
      ...newFiles,
      ...currentFiles.filter(file => !ids.has(file.name)),
    ];
    setFiles(merged);
  };
            ...
            return (
              <motion.div
                ...
                <div
                  ...
                  <span className="line-clamp-1 text-sm group-hover:text-primary">
                    {fileName}
                  </span>
        ...
        <AnimatePresence>
          <div
            ...
            <UploadCloudIcon
              className={cn(
                'size-20 md:size-32',
                !isDragging ? 'stroke-gray-300' : 'stroke-primary',
              )}
              strokeWidth={1.5}
            />
            <p
