...
import {CustomTable, ScreenLoader} from '@/components';
import {APP_ROUTES, DEFAULT_COUPON_IMAGE, DEFAULT_IMAGE} from '@/constants';
import {ECouponApplyType, ECouponType} from '@/lib/enums';
import {
...
import {ICategory, ICoupon, IPaginate, IProduct} from '@/lib/interfaces';
import {
  useCreateCouponMutation,
  useUpdateCouponMutation,
} from '@/services/coupon';
import {fetchPublishedProductsAction} from '@/services/product/product.action';
...
const CouponForm = ({type = 'new', data}: Props) => {
  ...
  const fetchNext = useCallback(async (page: number, query: any = {}) => {
    setIsFetching(true);
    const res = await fetchPublishedProductsAction({page, ...query});
    if (res.success) {
  ...
  const {
    ...
    resolver: zodResolver(
      isEdit ? updateCouponFormSchema : createCouponFormSchema,
    ),
  ...
  const updateCouponMutation = useUpdateCouponMutation(params.id?.toString());
    ...
    <div className="flex flex-wrap gap-8">
      <div className="space-y-1">
        ...
        <p className="text-sm text-slate-500">
          {isEdit ? 'Update' : 'Add'} coupon information from here
        </p>
      </div>
                ...
                return (
                  <div
                    ...
                    <Button
                      ...
                      onClick={() =>
                        setSelectedKeys(
                          selectedKeys.filter(key => key !== product._id),
                        )
                      }>
                      Remove
                    </Button>
