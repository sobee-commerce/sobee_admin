...
import {IOrderAnalyticData} from '@/lib/interfaces';
import {fetchOrderAnalyticsAction} from '@/services/analytics/analytics.action';
...
const OrderAnalytics = () => {
  ...
  const fetchOrderAnalyticsData = useCallback(async () => {
    const startDate = new Date(
      rangeDate.start.toDate(getLocalTimeZone()),
    ).toLocaleDateString();
    const endDate = new Date(
      rangeDate.end.toDate(getLocalTimeZone()),
    ).toLocaleDateString();
    setLoading(true);
    // fetch order analytics
    const res = await fetchOrderAnalyticsAction(startDate, endDate);
    if (res.success) {
      const data = res.data!;
      ...
      <div className="mb-5 flex items-center justify-between">
        ...
        <div>
          <DateRangePicker
            ...
            onChange={setRangeDate as any}
            label="Range Order Date"
            isDisabled={loading}
          />
        </div>
      </div>
