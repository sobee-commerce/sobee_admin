...
import {IPaginate, IReview} from '@/lib/interfaces';
import {fetchAllReviewsACtion} from '@/services/review/review.action';
...
const ReviewListData = () => {
  ...
  const fetchNext = useCallback(async (page: number, query: any = {}) => {
    setIsFetching(true);
    const res = await fetchAllReviewsACtion({page, ...query});
    if (res.success) {
  ...
  return (
    <div>
      <CustomTable
        ...
        RenderCell={(review, columnKey) => (
          <RenderCellReview review={review} columnKey={columnKey} />
        )}
