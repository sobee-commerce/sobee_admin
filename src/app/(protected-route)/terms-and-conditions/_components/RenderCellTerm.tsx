...
import {ITerm, IUser} from '@/lib/interfaces';
import {useDeleteTermMutation} from '@/services/term';
import {
  Button,
  Chip,
  Link,
...
const RenderCellTerm = ({term, columnKey}: Props) => {
  const cellValue = term[columnKey as keyof ITerm] as any;
  const [showPopover, setShowPopover] = useState(false);
  const [showModal, setShowModal] = useState(false);

  const deleteTermMutation = useDeleteTermMutation();

  const isLoading = deleteTermMutation.isPending;

  const onDelete = () => {
    deleteTermMutation.mutate(term._id!, {
      onSuccess: data => {
        if (data.success) {
      ...
      return (
        <div className="flex items-center gap-1">
          ...
          <Button
            ...
            color="primary"
            as={Link}
            href={APP_ROUTES.TERMS_AND_CONDITIONS.EDIT.replace(
              ':id',
              term?._id!,
            )}>
            <SquarePen size={20} />
