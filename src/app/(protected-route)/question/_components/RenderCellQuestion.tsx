...
import {IProduct, IQuestion, IUser} from '@/lib/interfaces';
...
const RenderCellQuestion = ({question, columnKey}: Props) => {
  const cellValue = question[columnKey as keyof IQuestion] as any;
  const [showPopover, setShowPopover] = useState(false);
  const [showReplyModal, setShowReplyModal] = useState(false);

  const deleteQuestionMutation = useDeleteQuestionMutation();

  const isLoading = deleteQuestionMutation.isPending;
  ...
  switch (columnKey as QuestionColumnKey) {
    ...
    case 'question_and_answer':
      return (
        <div>
          ...
          {question.answer && (
            <p>
              A: {question.answer.content} ({question.answer.likes?.length || 0}{' '}
              likes)
            </p>
          )}
        </div>
    ...
    case 'createdAt':
      return (
        <p>{format(new Date(question.createdAt!), "dd/MM/yyyy 'at' HH:mm")}</p>
      );
    case 'actions':
      return (
