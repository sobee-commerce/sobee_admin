...
import {IFaq, IUser} from '@/lib/interfaces';
import {useDeleteFaqMutation} from '@/services/faq';
import {
  Button,
  Link,
  Popover,
...
const RenderCellFaq = ({faq, columnKey}: Props) => {
  const cellValue = faq[columnKey as keyof IFaq] as any;
  const [showPopover, setShowPopover] = useState(false);
  const [showModal, setShowModal] = useState(false);

  const deleteFaqMutation = useDeleteFaqMutation();

  const isLoading = deleteFaqMutation.isPending;
