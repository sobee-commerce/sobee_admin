...
const CategoryForm = ({type = 'new', data, categories}: Props) => {
  ...
  const {
    ...
    resolver: zodResolver(
      isEdit ? updateCategoryFormSchema : createCategoryFormSchema,
    ),
  ...
  const updateCategoryMutation = useUpdateCategoryMutation(data?._id!);

  const isLoading = createCategoryMutation.isPending;

  const onSubmit = (
    data: CreateCategoryFormSchema | UpdateCategoryFormSchema,
  ) => {
    const handler = isEdit ? updateCategoryMutation : createCategoryMutation;
    handler.mutate(data, {
      onSuccess: _data => {
        if (_data.success) {
